...
_checkout.processCheckout = function(data, cb) {
              ...
              const request = https.request(requestDetails, function(response) {
                ...
                if (status === 200) {
                  const itemsDetails = cartData.items.map(function(item, idx) {
                    return `${idx +
                      1}. ${item.displayName} || Quantity: ${item.qty} || Amount: ${item.price * item.qty}\n`;
                  });
                  const msg = `
  ...
  Total Amount Paid: ${cartData.amount}
...
_checkout.processPayment = function(cartData, paymentToken, cb) {
  const payload = {
    amount: cartData.amount,
    currency: "usd",
    description: "charge for order id: " + cartData.id,
  ...
  const request = https.request(requestDetails, function(response) {
    ...
    if (status === 200) {
      ...
      const msg = `
Your order number ${cartData.id} for USD ${cartData.amount / 100} has been paid.
...
-ADK PIZZA-
      `;
      helpers.mailGunJob(cartData.userEmail, msg, function(err) {
        if (!err) {
          cb(200);
        } else {
          console.log(err);
